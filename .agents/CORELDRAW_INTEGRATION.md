...
`scripts\Build-Release.ps1` produces `artifacts\CorelMate-0.1.0-docker.zip`. The package contains `CorelMate.UI.dll`, `CorelMate.Host.dll`, `CorelMate.Badges.dll`, `CorelMate.Infrastructure.dll`, `CorelMate.Core.dll`, `CorelMate.Curves.dll`, package-local session install/remove scripts, and a README. It deliberately excludes `Corel.Interop.CorelDRAW.dll` and `Corel.Interop.VGCore.dll`; CorelDRAW supplies those Corel-owned host assemblies. Source, tests, PDBs, local configuration, and developer scripts are also excluded.
